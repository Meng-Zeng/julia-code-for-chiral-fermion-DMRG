...
let
	...
	psi = read(f,"mps-$N-$G",MPS)
        ...
        correlation_A = zeros(ComplexF64,1,N-1)
        correlation_B = zeros(ComplexF64,1,N-1)
        #correlation_AB = zeros(ComplexF64,1,N-1)
        ...
        for d = 1:(N-1)
                ...
                corr3A = AutoMPO()
                corr3B = AutoMPO() 
                corr3A += "C3d",4*(4*(i-1)+1-1)+1, "C3",4*(4*(i+d-1)+1-1)+1
                corr3B += "C3d",4*(4*(i-1)+2-1)+1, "C3",4*(4*(i+d-1)+2-1)+1
                Corr3A = MPO(corr3A,sites)
                Corr3B = MPO(corr3B,sites)
                correlation_A[d] = inner(psi,Corr3A,psi)
                correlation_B[d] = inner(psi,Corr3B,psi)
        end

        ############# save to csv file ############
        writedlm("corr-A-$N-D8000-$G-new.csv",correlation_A)
        writedlm("corr-B-$N-D8000-$G-new.csv",correlation_B)
